...
// Anchored on Linux PATH_MAX, which is the most permissive of the supported backups stores'
...
func validateBootstrapSnapshot(snapshot string) error {
	...
	if !filepath.IsLocal(snapshot) {
		return errors.New("bootstrap snapshot must be a local path without parent-directory references")
	}
